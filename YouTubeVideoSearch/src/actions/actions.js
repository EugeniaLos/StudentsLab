...
import { fetchNextYoutubeVideo } from "../service/service";
...
export const fetchNextVideo = (searchText, nextPageToken) => async dispatch => {
  const searchResult = await fetchNextYoutubeVideo(searchText, nextPageToken);
  let searchResultJson = await searchResult.json();
  let videos = [];
  if (await searchResultJson.items) {
    let items = searchResultJson.items;
    let video;
    items.forEach(function(item) {
      video.id = item.id.videoId;
  ...
  dispatch({ type: "FETCH_NEXT_VIDEO_SUCCESS", payload: { videos } });
};
